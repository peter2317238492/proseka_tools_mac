    ...
    static inline std::vector<UserAreaActionSets> fromJsonList(const json& jsonData) {
        ...
        for (const auto& item : jsonData) {
            ...
            if (item.is_object()) {
                ...
                if (item.size() > 0 && item[0].is_number_integer()) actionSet.id = item[0].get<int>();
                if (item.size() > 1 && item[1].is_string()) actionSet.status = mapEnum(EnumMap::actionSetStatus, item[1].get<std::string>());
            }
